...
export default function ServicesView() {
  ...
  const handleSaveService = (serviceData: { id?: number; name: string; duration: string; price: string; type: string }) => {
    if (serviceData.id) {
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {services.map(service => {
            ...
            return (
              <div key={service.id} className={`bg-card rounded-3xl border ${isActive ? 'border-border shadow-sm hover:border-[#C0987A]' : 'border-border/40 opacity-60'} p-6 transition-all relative group flex flex-col`}>
                ...
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 ${isActive ? 'bg-muted text-[#C0987A]' : 'bg-muted/50 text-muted-foreground'}`}>
                ...
                <div className="mt-auto pt-5 border-t border-border flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-bold uppercase tracking-wider ${isActive ? 'text-[#C0987A]' : 'text-muted-foreground'}`}>
          ...
          <button
            onClick={() => setServiceModalOpen(true)}
            className="bg-transparent border-2 border-dashed border-border rounded-3xl p-6 flex flex-col items-center justify-center text-[#C0987A] hover:bg-muted/20 hover:border-[#C0987A] transition-all min-h-[280px] cursor-pointer"
          >
            <div className="w-14 h-14 rounded-full bg-muted flex items-center justify-center mb-4">
              <Plus className="w-6 h-6 text-[#C0987A]" />
            </div>
